...
IMPLICIT NONE
	...
	DO K=0,Kp
		DO J=1,JMAX
			DO I=1,IMAX
!				F(i,j,k) = F(i,j,k) + ( Feq(i,j,k) - F(i,j,k) )*Ta1
...
IMPLICIT NONE
	...
	do J=1,JMAX
		do I=1,IMAX
			...
			U(i,j) = ( F(i,j,1) - F(i,j,3) + F(i,j,5) - F(i,j,6) - F(i,j,7) + F(i,j,8) )/P0
			V(i,j) = ( F(i,j,2) - F(i,j,4) + F(i,j,5) + F(i,j,6) - F(i,j,7) - F(i,j,8) )/P0 
		enddo
	enddo
...
!--------- outlet
	U(IMAX,:) = ( 4.d0*U(IMAX-1,:)-U(IMAX-2,:) )/3.d0
